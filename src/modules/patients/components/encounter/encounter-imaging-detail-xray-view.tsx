...
export function EncounterImagingDetailXRayView({
  ...
  const { diagnosticReportXRay, encounter, patient } = data
      ...
      <div className="rounded-lg border border-border bg-card p-6 ">
        ...
        <div className="flex items-center gap-3 pb-5 border-b border-border">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Mã phiếu: <span className="font-mono font-semibold">{diagnosticReportXRay.orderCode}</span> • Lượt khám: {encounter.encounterCode}
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5 rounded-lg bg-muted/30 p-4 border border-border/60 text-xs">
          ...
          <div>
            ...
            <p className="font-semibold text-foreground mt-0.5">{diagnosticReportXRay.orderingPhysician}</p>
          </div>
          <div>
            <p className="text-[11px] text-muted-foreground font-medium">Bác sĩ đọc KQ</p>
            <p className="font-semibold text-foreground mt-0.5">{diagnosticReportXRay.radiologist}</p>
          </div>
          <div>
            <p className="text-[11px] text-muted-foreground font-medium">Thời gian chụp</p>
            <p className="font-semibold text-foreground mt-0.5">{diagnosticReportXRay.performedAt}</p>
          </div>
          <div>
            <p className="text-[11px] text-muted-foreground font-medium">Thiết bị chụp</p>
            <p className="font-semibold text-foreground mt-0.5 truncate" title={diagnosticReportXRay.device}>
              {diagnosticReportXRay.device}
            </p>
          </div>
          <div>
          ...
          <div className="lg:col-span-6 space-y-5">
            ...
            <div className="rounded-lg border border-border bg-card p-5 text-xs">
              ...
              <ul className="space-y-2.5 text-muted-foreground list-disc pl-4 leading-relaxed">
                {diagnosticReportXRay.descriptionPoints.map((point: string, index: number) => (
            ...
            <div className="rounded-lg border border-border bg-muted/20 p-4 text-xs flex items-center justify-between">
              <div>
                ...
                <p className="font-bold text-foreground text-sm mt-0.5">
                  {diagnosticReportXRay.radiologist}
                </p>

...
export function PrintPrescriptionDialog({
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-3xl rounded-2xl border border-border bg-card shadow-2xl overflow-hidden my-6">
        ...
        <div className="p-6 bg-muted/20 max-h-[70vh] overflow-y-auto flex justify-center">
          <div className="w-full max-w-2xl bg-white text-slate-900 border border-slate-200 rounded-xl p-8 shadow-xs font-sans text-xs">
            {/* Clinic Brand Header */}
            ...
            {/* Patient Details */}
            <div className="grid grid-cols-2 gap-y-2 py-3 border-y border-slate-100 text-xs">
              <div>
                <span className="text-slate-500">Họ và tên: </span>
                <span className="font-bold uppercase text-slate-900">{patient.fullName}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="font-mono font-bold text-slate-900">{patient.patientCode}</span>
              </div>
              <div>
                <span className="text-slate-500">Điện thoại: </span>
                <span className="font-mono font-medium text-slate-900">{patient.phoneNumber}</span>
              </div>
            ...
            {/* Doctor Note */}
            <div className="mt-5 rounded-lg bg-blue-50/60 p-3 border border-blue-100 text-xs">
              <span className="font-bold text-blue-900">Lời dặn của bác sĩ: </span>
              <span className="text-slate-700">{prescriptions.doctorNotes}</span>
            </div>

            {/* Signatures */}
            <div className="mt-8 grid grid-cols-2 text-center text-xs">
              <div className="space-y-1">
                <p className="font-medium text-slate-500">Bệnh nhân / Người nhận</p>
                <p className="text-[10px] text-slate-400 italic">(Ký và ghi rõ họ tên)</p>
              </div>
              <div className="space-y-1">
                <p className="text-slate-500">Hà Nội, ngày 16 tháng 09 năm 2024</p>
                <p className="font-bold text-slate-900">Bác sĩ khám bệnh</p>
                <p className="text-[10px] text-slate-400 italic pb-12">(Ký và ghi rõ họ tên)</p>
                <p className="font-bold text-slate-900">{encounter.physicianName}</p>
              </div>
            </div>
          </div>
        </div>

...
describe("CreateHealthExaminationBatchDialog (Screen 03 – Tạo đợt khám mới)", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it("1 & 2: opens modal with correct organization name in title and subtitle", async () => {
    renderWithClient(
  ...
  it("6 & 7: validates unitPrice > 0 for selected services", async () => {
    const user = userEvent.setup()
  ...
  it("8, 9 & 10: successfully creates batch, sends only selected services, closes modal, and navigates to batch detail", async () => {
    const user = userEvent.setup()
    ...
    // 4. Verify API called with only the 2 selected services
    await waitFor(() => {
    ...
    // Unchecked services are NOT sent in the payload
    expect(payload.services.some((i: { serviceId: string }) => i.serviceId === "item-xnnt")).toBe(
      false
    )
  ...
  it("11: preserves user entered data and shows error alert when submit fails", async () => {
    const user = userEvent.setup()
    const handleOpenChange = vi.fn()
    vi.spyOn(healthBatchesApi, "createHealthExaminationBatch").mockRejectedValueOnce(
  ...
  it("12: disables submit button while pending to prevent double-submit", async () => {
    const user = userEvent.setup()
    let resolvePromise!: (val: HealthExaminationBatch) => void
    const pendingPromise = new Promise<HealthExaminationBatch>((resolve) => {
      resolvePromise = resolve
    })

    vi.spyOn(healthBatchesApi, "createHealthExaminationBatch").mockReturnValueOnce(
      pendingPromise
    )

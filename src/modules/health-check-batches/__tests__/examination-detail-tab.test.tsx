...
describe("Screen 05 – Tab 'Chi tiết khám' (Exam Batch Detail)", () => {
  ...
  it("1 & 2: switches from 'Nhân sự khám' to 'Chi tiết khám', updates header subtitle, hides header actions, without page reload", async () => {
    ...
    // Initial default tab is 'Nhân sự khám'
    expect(
      screen.getByText("Danh sách nhân viên tham gia đợt khám")
    ).toBeInTheDocument()
    expect(screen.getByRole("button", { name: /Import nhân sự/i })).toBeInTheDocument()
    ...
    // Subtitle updates to 'Theo dõi nhân sự đã khám các hạng mục nào'
    expect(
  ...
  it("3, 4, 5, 6: renders dynamic category columns from batch items, COMPLETED as 'X', PENDING/SKIPPED as blank", async () => {
    const sampleCategories: MatrixCategoryItem[] = [
      { id: "cat-1", name: "Khám nội" },
      { id: "cat-2", name: "XN máu" },
      { id: "cat-3", name: "Chuyên khoa khác" },
    ]

    const sampleEmployees: EmployeeMatrixItem[] = [
      {
        id: "emp-1",
        employeeCode: "NV001",
  ...
  it("7: renders reference image data for rows 1 to 10 with exact completed X marks and badges", async () => {
    ...
    // Check employees matching reference image
  ...
  it("8: searches employees by code, name, and CCCD with debounce", async () => {
    const user = userEvent.setup()
  ...
  it("9: filters employees by department", async () => {
    const user = userEvent.setup()
    ...
    const deptSelect = screen.getByRole("combobox", { name: /Phòng ban/i })
    expect(deptSelect).toBeInTheDocument()
  })

  it("10: filters employees by examination status", async () => {
    const user = userEvent.setup()
  ...
  it("12: displays pagination info 'Hiển thị 1 - 10 của 50 nhân sự' and page navigation", async () => {
    const user = userEvent.setup()
  ...
  it("13: handles empty search result state gracefully", async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText("Không tìm thấy nhân sự phù hợp với điều kiện tìm kiếm.")
        ).toBeInTheDocument()
      },
      { timeout: 2000 }
    )
  ...
  it("14: shows helper notice when employees exist but no completed exams", () => {
    const sampleCategories: MatrixCategoryItem[] = [
      { id: "cat-1", name: "Khám nội" },
    ]

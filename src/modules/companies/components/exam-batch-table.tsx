...
import { ExamBatch } from "@/modules/health-check-batches"

interface ExamBatchTableProps {
  enterpriseId: string
  batches: ExamBatch[]
  isLoading?: boolean
  onCreateClick?: () => void
...
export function ExamBatchTable({
  enterpriseId,
  batches,
  isLoading = false,
  onCreateClick,
  ...
  const handleRowClick = (batchId: string) => {
    router.push(`/enterprises/${enterpriseId}/exam-batches/${batchId}`)
  }

...
export function ExaminationItemPriceTable({
  ...
  // Watch current items to get reactive selected and unitPrice states
  const watchedItems = watch("items") || []

  // Global items error (e.g. at least 1 item selected)
  const itemsError =
    errors.items?.message ||
    (errors.items as unknown as { root?: { message?: string } })?.root?.message
  ...
  return (
    <div className="space-y-3">
            ...
            <tbody>
              {isLoading ? (
                ...
                fields.map((field, index) => {
                  ...
                  const defaultPrice =
                    masterItems.find(
                      (m) => m.id === currentItem.examinationItemId
                    )?.defaultPrice || 0
      ...
      {/* Validation message if no items are selected */}
      {itemsError && typeof itemsError === "string" && (

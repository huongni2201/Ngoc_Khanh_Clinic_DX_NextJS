...
describe("CreateHealthExaminationBatchDialog (Screen 03 – Tạo đợt khám mới)", () => {
  ...
  it("8, 9 & 10: successfully creates batch, sends only selected items, closes modal, and navigates to batch detail", async () => {
    ...
    expect(payload.items.map((i: { examinationItemId: string }) => i.examinationItemId)).toEqual([
      "item-kntq",
      "item-xnm",
    ])
    // Unchecked items are NOT sent in the payload
    expect(payload.items.some((i: { examinationItemId: string }) => i.examinationItemId === "item-xnnt")).toBe(
      false
    )
    ...
    await waitFor(() => {
      ...
      expect(mockPush).toHaveBeenCalledWith(
        expect.stringMatching(/^\/enterprises\/ent-2\/health-examination-batches\/batch-/)
      )
    })
  })

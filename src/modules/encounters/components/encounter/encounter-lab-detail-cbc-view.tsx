...
import { EncounterDetailData, LabResult } from "../../types/encounter"

interface EncounterLaboratoryDetailViewProps {
  data: EncounterDetailData
  onBack: () => void
}

export function EncounterLaboratoryDetailView({ data, onBack }: EncounterLaboratoryDetailViewProps) {
  const { laboratoryReport, encounter, patient } = data
      ...
      <div className="rounded-lg border border-border bg-card p-6 ">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-status-success-bg px-2 py-0.5 text-xs font-semibold text-status-success border border-status-success/30">
                ...
                {laboratoryReport.statusLabel}
              </span>
            </div>
            <p className="text-xs text-muted-foreground">
              Mã xét nghiệm: <span className="font-mono font-semibold">{laboratoryReport.orderCode}</span> • Lượt khám: {encounter.encounterCode}
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5 rounded-lg bg-muted/30 p-4 border border-border/60 text-xs">
          <div>
            ...
            <p className="font-semibold text-foreground mt-0.5">{laboratoryReport.orderingPhysician}</p>
          </div>
          <div>
            <p className="text-[11px] text-muted-foreground font-medium">Người lấy mẫu</p>
            <p className="font-semibold text-foreground mt-0.5">{laboratoryReport.sampleCollector}</p>
          </div>
          <div>
            <p className="text-[11px] text-muted-foreground font-medium">Thời gian lấy mẫu</p>
            <p className="font-semibold text-foreground mt-0.5">{laboratoryReport.sampledAt}</p>
          </div>
          <div>
            <p className="text-[11px] text-muted-foreground font-medium">Thời gian trả KQ</p>
            <p className="font-semibold text-foreground mt-0.5">{laboratoryReport.resultReportedAt}</p>
          </div>
          <div>
            <p className="text-[11px] text-muted-foreground font-medium">Bác sĩ duyệt KQ</p>
            <p className="font-semibold text-foreground mt-0.5">{laboratoryReport.approvingPhysician}</p>
          </div>
          <div>
        ...
        <div className="mt-6 overflow-x-auto rounded-lg border border-border">
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-border">
              {laboratoryReport.indicators.map((ind: LabResult) => (
                <tr key={ind.sequence} className="hover:bg-muted/20 transition-colors">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 text-xs">
            ...
            <p className="mt-1.5 font-medium text-foreground leading-relaxed">
                {laboratoryReport.conclusion}
            </p>
            <p className="mt-2 text-[11px] text-muted-foreground">
              Ghi chú mẫu: {laboratoryReport.notes}
            </p>
          </div>
          ...
          <div className="rounded-lg border border-border bg-muted/20 p-4 text-xs flex flex-col justify-between">
            ...
            <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
              <div>
                ...
                <p className="font-bold text-foreground text-sm">{laboratoryReport.approvingPhysician}</p>
              </div>
              <span className="rounded bg-muted px-2 py-1 text-[10px] font-mono text-muted-foreground">
                Ký số lúc {laboratoryReport.resultReportedAt}
              </span>
            </div>
          </div>
        </div>

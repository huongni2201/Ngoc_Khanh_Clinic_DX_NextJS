 ...
 * Mỗi row = 1 nhân sự
 * Columns: Mã người khám, Họ tên, CCCD, Đơn vị công tác, các dịch vụ lâm sàng...
...
export function generateDetailHorizontalCSV(data: ExportDetailMatrixData): string {
  ...
  for (const row of data.rows) {
    const itemMarks = data.columns.map((col) =>
      row.completedServiceIds.includes(col.id) ? "X" : ""
    )

    const rowData = [
      `"${row.participantCode.replace(/"/g, '""')}"`,
      `"${row.fullName.replace(/"/g, '""')}"`,
      `"${row.identificationNumber.replace(/"/g, '""')}"`,
      `"${row.organizationUnit.replace(/"/g, '""')}"`,
      ...itemMarks,
    ]

...
let participantsStore: HealthExaminationParticipant[] = generateSeedParticipants("batch-1")
...
export async function fetchHealthExaminationBatchReport(batchId: string): Promise<HealthExaminationBatchReportSummary> {
  ...
  // If batch has no services or 0 participants, return empty items
  if (items.length === 0 || batch.participantCount === 0) {
    return {
      batchId,
      items: [],
      totalAmount: 0,
...
// ----------------------------------------------------
// Screen 04: Import Participants Mutation
// ----------------------------------------------------
export async function importParticipantsToBatch(
  batchId: string,
  participants: HealthExaminationParticipant[]
): Promise<{ count: number }> {
  await new Promise((resolve) => setTimeout(resolve, 300))

  // Replace or append participants
  const otherParticipants = participantsStore.filter((e) => e.batchId !== batchId)
  participantsStore = [...otherParticipants, ...participants]

  // Update batch participant count
  const batch = healthExaminationBatchesStore.find((b) => b.id === batchId)
  if (batch) {
    batch.participantCount = participants.length
  }

  return { count: participants.length }
}

export async function populateSampleParticipantsForBatch(
  batchId: string
): Promise<{ count: number }> {
  await new Promise((resolve) => setTimeout(resolve, 200))
  const newParticipants = generateSeedParticipants(batchId)
  return importParticipantsToBatch(batchId, newParticipants)
}

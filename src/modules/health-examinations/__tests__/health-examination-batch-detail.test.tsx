...
describe("HealthExaminationBatchDetailPage (Screen 04 – Chi tiết đợt khám)", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it("renders header, breadcrumbs, title, and action buttons without global export/report buttons", async () => {
    ...
    // Breadcrumbs
    expect(screen.getByRole("link", { name: "Đơn vị" })).toBeInTheDocument()
    expect(screen.getByText("Đợt khám")).toBeInTheDocument()

    // Top-right buttons
    const importBtn = screen.getByRole("button", { name: /Import người khám/i })
  ...
  it("renders Tab 1 (Người khám) by default with participant fields, status badges, and pagination", async () => {
    renderWithClient(
      <HealthExaminationBatchDetailPage organizationId="ent-2" batchId="batch-1" />
    )
    ...
    await waitFor(() => {
      expect(screen.getByText("Mã người khám")).toBeInTheDocument()
    })
    ...
    expect(
      screen.getByRole("button", {
        name: /Thao tác với người khám Trần Minh Đức/i,
      })
    ).toBeInTheDocument()

    // Row 3: FPT003 Lê Quang Huy, Row 7: FPT007 Vũ Thị Thanh Huyền (both missing identification numbers)
    expect(screen.getByText("FPT003")).toBeInTheDocument()
    expect(screen.getByText("Lê Quang Huy")).toBeInTheDocument()
    expect(screen.getAllByText("Thiếu số định danh").length).toBe(2)
  ...
  it("opens the Import Participants Dialog when clicking [Import người khám]", async () => {
    const user = userEvent.setup()
    ...
    // Dialog title
    expect(
      screen.getByRole("heading", { name: "Import danh sách người khám" })
    ).toBeInTheDocument()

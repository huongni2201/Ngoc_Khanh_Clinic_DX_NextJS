...
describe("Screen 06 – Tab Báo cáo của màn Chi tiết đợt khám", () => {
  ...
  it("generates correct horizontal Excel matrix with X markers for completed items", async () => {
    const exportData = await fetchExaminationDetailExportData("batch-1")
    ...
    expect(row1.participantCode).toBe("FPT001")
    expect(row1.completedServiceIds.length).toBe(7)

    const csvContent = generateDetailHorizontalCSV(exportData)
    // Check BOM
    expect(csvContent.startsWith("\uFEFF")).toBe(true)
    // Check headers
    expect(csvContent).toContain("Mã người khám,Họ tên,CCCD,Đơn vị công tác")
  ...
  it("generates correct vertical Excel summary with items and grand total", async () => {
    const summaryData = await fetchExaminationSummaryExportData("batch-1")
  ...
  it("switches to Tab 3 (Báo cáo), updates header subtitle, hides header actions, and displays exact UI", async () => {
    ...
    // On Tab 1 by default: header buttons exist, subtitle is for participants
    expect(
      screen.getByText("Danh sách người khám tham gia đợt khám")
    ).toBeInTheDocument()
    expect(
      screen.getByRole("button", { name: /Import người khám/i })
    ).toBeInTheDocument()
    expect(
